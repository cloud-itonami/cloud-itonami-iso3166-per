...
2. Register the client's intake: business type, target public function,
   prior filing history in Peru if any.
3. Run the advisor in read-only mode against SEACE (Sistema Electrónico de Contrataciones del Estado), Peru's official electronic public procurement system.
...
- named referral relationship with Peruvian-licensed counsel or a
...
a working referral relationship with Peruvian-licensed counsel or a
registered agent for whatever licensed representation the law of
Peru requires for actual public-procurement filings.
