(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Peru-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Peru-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-pe-entity?
                                       true` (near-universal for Peru
                                       ...
                                       Grounded in Peruvian legal entity (RUC)
                                       typically required for SEACE awards
                                       (contracting authorities / OSCE).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(RUC/統一資格/SEACE登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- pe-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-pe-entity? true`, INDEPENDENTLY verify
  `:has-pe-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-pe-entity?` ground truth (most Peru public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-pe-entity? e))
                 (not (true? (:has-pe-entity? e))))
        [{:rule :pe-entity-missing
          :detail (str subject " はペルー居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ruc-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ruc? true`, INDEPENDENTLY check
  `:ruc-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (pe-entity-missing-violations request st)
