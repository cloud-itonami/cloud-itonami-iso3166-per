  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems PE"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems PE" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest pe-entity-missing-is-held-and-unoverridable
  (testing "missing Japan-resident authorized representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:pe-entity-missing} (-> (store/ledger db) last :basis)))
